...
OLD2_ORIG = b'supportedReasoningEfforts:B?["off","low","medium","high"]:["none"],defaultReasoningEffort:R.reasoningEffort'
OLD2_PARTIAL = b'supportedReasoningEfforts:B?["off","low","medium","high","max"]:["none"],defaultReasoningEffort:R.reasoningEffort'
NEW2 = b'supportedReasoningEfforts:B?R.provider=="openai"?["none","low","medium","high","xhigh"]:["off","low","medium","high","max"]:["none"],defaultReasoningEffort:R.reasoningEffort'

if NEW2 in data:
    print("mod9 路径2 ($A): 已应用，跳过")
elif OLD2_PARTIAL in data:
    data = data.replace(OLD2_PARTIAL, NEW2, 1)
    diff2 = len(NEW2) - len(OLD2_PARTIAL)
    total_diff += diff2
    print(f"mod9 路径2 ($A): 从 max-only 升级为 provider-aware ({diff2:+d} bytes)")
elif OLD2_ORIG in data:
    data = data.replace(OLD2_ORIG, NEW2, 1)
    diff2 = len(NEW2) - len(OLD2_ORIG)
    total_diff += diff2

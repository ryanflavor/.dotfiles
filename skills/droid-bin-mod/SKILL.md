...
mod10: 适配 Opus 4.7 adaptive thinking → 多轮保留 thinking 配置不被剥离
...
注：mod10 跳过 P7I 内 I7I/D7I 历史检测，保留 `{type:"adaptive"}` 跨轮生效
...
| 7   | effort 级别  | `["off","low","medium","high"]` | 按 provider 区分 | +74/处 | anthropic 加 xhigh+max，openai 加 xhigh |
...
| 10  | adaptive thinking | `if(I7I(B)\|\|D7I(B))` | `if(!1            )` | 0 | P7I 内跳过历史检测，保留 adaptive 多轮 |
| 补偿 | 死代码区域   | 多处死代码   | 注释/缩短填充   | -N   | 统一补偿 mod3+mod7 的增量             |
...
- mod10: `P7I` 内 `if(V.thinking){if(I7I(B)||D7I(B))` 的内层条件短路为 `!1 + 空格`，跳过 Opus 4.7 `type:"adaptive",display:"summarized"` 响应触发的误剥离

### 修改 10: Opus 4.7 adaptive thinking 多轮保留

**问题**: Opus 4.7 的 `{type:"adaptive", display:"summarized"}` 模式下，assistant 响应首块通常直接是 `tool_use`（不是 `type:"thinking"`）。droid 的 P7I 检测到"历史中存在 assistant 消息但不以 thinking 开头"(I7I 返回 true)，会把当前请求的 thinking 配置剥掉，并从历史中删除所有 thinking 块。结果：第二轮起 thinking 永久关闭，adaptive 预算自适应失效。
...
**修改**: `I7I(B)||D7I(B)` → `!1            ` (字节中性：`!1` + 12 空格 = 14 字节)

**效果**: 历史检测分支永不执行，thinking 配置跨轮保留，adaptive 多轮生效。

**字节**: 0 bytes（同长替换）

**稳定锚点**: `V.thinking){if(V(V)||V(V))` —— 函数调用 `||` 结构 + `.thinking` 属性名，变量名由混淆器决定，脚本用正则匹配。

**副作用**: 对 legacy `type:"enabled"` 模式也跳过历史剥离。若历史中存在不一致 thinking 块且切换到 enabled 模式，可能引发 Anthropic 一致性报错（实测未见）。如需更保守，可改为条件判断 `T.thinking.type=="enabled"` 的 +N 字节版本。
...
mods/mod3_output_lines.py                # 输出行数 4/8→99 (v0.49:+1, v0.74:0, v0.104:+4)
...
mods/mod7_custom_effort_levels.py        # custom model effort 级别 (+74/匹配, 通常 +148 bytes)
mods/mod8_disable_builtin_models.py      # 禁用内置模型，只允许 BYOK (0 bytes)
mods/mod9_disable_auto_update.py         # 禁用自动更新 (0 bytes)
mods/mod10_adaptive_thinking.py          # Opus 4.7 adaptive thinking 多轮保留 (0 bytes)
```

mod3 产生 +1~+4 bytes，mod7 通常 +148 bytes（2 处 ×74），其他 mod 都是 0 bytes。
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod3_output_lines.py              # +1~+4 bytes
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod7_custom_effort_levels.py      # +148 bytes
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod10_adaptive_thinking.py        # 0 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/compensations/comp_universal.py 152  # 补偿 mod3(+4) + mod7(+148)
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod10_adaptive_thinking.py
```

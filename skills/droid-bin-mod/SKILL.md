...
mod3:  命令输出截断行数 4 行 → 99 行 (同时取消输出区 Ctrl+O 提示)
...
mod10: Compaction 默认模型 factory-default → current-model (BYOK 压缩不扣 Factory token)

注：mod3 同时控制输出行数和 "output truncated" 提示 (同一变量)
注：mod6 配合 settings.json 中 missionModelSettings 设置 Worker/Validator 模型
注：mod7 修复 custom model Tab 切换只有 off/low/medium/high 的限制
注：mod8 修改 validateModelAccess，非 custom: 前缀的模型全部返回 allowed:false
注：mod10 修改 getCompactionModelMode() 默认回退值，避免 BYOK 用户压缩时触发 Factory 402
...
mod3 使用同一个变量同时控制输出行数与 Ctrl+O 提示：`aGR=4`
- `slice(0,aGR)` - 截取前 aGR 行显示
- `D>aGR&&` - 超过 aGR 行才显示提示

修改 `aGR=4` → `aGR=99` 一次性解决行数 + 提示两件事，+1 byte 变化（4→99 多一位数字），需要补偿。
...
- 变量 `aGR` 同时控制：
  - `slice(0,aGR)` - 显示前多少行
  - `D>aGR&&` - 超过多少行显示 Ctrl+O 提示
- 修改变量定义，一次性解决输出行数 + 提示两件事
...
| 3   | 输出行数+提示 | `aGR=4`      | `aGR=99`       | +1   | 输出显示 99 行，超过才显示 Ctrl+O 提示   |
| 4   | diff 行数    | `LD=20`      | `LD=99`        | 0    | Edit diff 显示 99 行                      |
| 5   | model cycle  | peek/cycle 函数 | 覆盖H+移除检查  | 0    | Ctrl+N 只切换 custom model                |
| 6   | mission 模型 | `Y9H.includes(X)` | `!0` + 空格填充 | 0  | 改条件而非数据，不强切+不警告         |
| 7   | effort 级别  | `["off","low","medium","high"]` | 按 provider 区分 | +66 | anthropic 加 max，openai 加 xhigh |
| 8   | 禁用内置模型 | `allowed:!0` | `allowed:!1`     | 0    | validateModelAccess 非 custom 模型拒绝 |
| 9   | 禁用自动更新 | `let H,{remoteConfig:$}=this.config;` | `return null;/*..*/` | 0 | checkForUpdates() 直接返回 null |
| 10  | 压缩默认模型 | `??"factory-default"` | `??"current-model"  ` | 0 | getCompactionModelMode() 默认走 BYOK |
| 补偿 | 死代码区域   | 多处死代码   | 注释/缩短填充   | -N   | 统一补偿 mod3(+1) + mod7(+66/+132)    |

**注**：
- mod1: 命令框提示（command truncated）
- mod3: 输出区行数和 Ctrl+O 提示由同一变量控制
- mod5: 修改 `peekNextCycleModel`, `peekNextCycleSpecModeModel`, `cycleSpecModeModel` 三个函数
  （`cycleModel` 是委托函数，无需修改）
- mod6: 两处 `Y9H.includes(X)` → `!0`（改条件，不改数据结构）
- mod7: wR() 中 custom model 分支，根据 T.provider 返回正确的 effort 列表
- mod8: validateModelAccess 最终 return `allowed:!0` → `!1`，禁用所有非 custom 模型
- mod9: `checkForUpdates()` 函数体开头插入 `return null;`，跳过版本检查
- mod10: `getCompactionModelMode()` fallback 从 `"factory-default"` 改为 `"current-model"`，
  避免压缩走 Factory Sonnet 4.5（扣 Factory token，BYOK 用户触发 402）
...
**配合 mod7**: 应用 mod7+mod6 后，检查 `~/.factory/settings.json` 配置完整性，
...
**配合 mod7**: 应用 mod7 后，检查 `~/.factory/settings.json` 中 `extraArgs` 的 effort 相关参数。
mod7 解锁了完整 effort 级别后，extraArgs 中的 effort 参数已冗余，且会导致副作用。
...
mods/mod3_output_lines.py                # 输出行数 aGR=4→99 (+1 byte, 同时解决 Ctrl+O 提示)
mods/mod4_diff_lines.py                  # diff 行数 20→99 (0 bytes)
mods/mod5_custom_model_cycle.py          # Ctrl+N 只切换 custom model (0 bytes)
mods/mod6_mission_model.py               # Mission 模型不强切 (0 bytes)
mods/mod7_custom_effort_levels.py        # custom model effort 级别 (+66 bytes)
mods/mod8_disable_builtin_models.py      # 禁用内置模型，只允许 BYOK (0 bytes)
mods/mod9_disable_auto_update.py         # 禁用自动更新 (0 bytes)
mods/mod10_compaction_current_model.py   # 压缩默认走 BYOK (0 bytes)
```

mod3 产生 +1 byte，mod7 产生 +66 bytes（0.69+ 有 2 处则 +132），其他 mod 都是 0 bytes。
由 `comp_universal.py N` 统一补偿（N = 实际增加字节数）。
...
用法：`python3 comp_universal.py N` 补偿 mod3(+1) + mod7(+66 或 +132) 的增量。

补偿区域来源 (由各 mod 短路/替换产生的死代码):
- FFH 死代码 (mod1 短路): ~151B — 截断函数被短路后的不可达代码，替换为 `;return{text:H,isTruncated:!1}`
- mod6 else 分支: ~44B — enter-mission 永不执行的 else
- mod6 空格填充: ~25B — !0 替换后的空格
- mod5 注释: ~36B — validateModelAccess 注释掉的检查
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod5_custom_model_cycle.py
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod6_mission_model.py
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod7_custom_effort_levels.py      # +66 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod8_disable_builtin_models.py    # 0 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod9_disable_auto_update.py       # 0 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod10_compaction_current_model.py # 0 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/compensations/comp_universal.py 133  # 补偿 mod3(+1) + mod7(+132)
...
```bash
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod9_disable_auto_update.py
```

**原理**: patch `Zp.checkForUpdates()` 方法，在函数体开头插入 `return null;`，
使版本检查永远返回"无更新"。0 字节变化，无需补偿。

配合 `sudo chattr +i ~/.local/bin/droid`（Linux）双保险锁定。
...
**mod8** (禁用内置模型):
```bash
# 检查 validateModelAccess 的 non-custom return
strings ~/.local/bin/droid | grep -c 'isCustomModel:!1};return{allowed:!0'
strings ~/.local/bin/droid | grep -c 'isCustomModel:!1};return{allowed:!1'
```

### 修改 8: 禁用 droid 官方内置模型

**目标**: `/model` 菜单和 Ctrl+N 只能选择 BYOK custom model，droid 内置模型不可用。
...
validateModelAccess(H) {
  let $ = this.getModelPolicy();
  if (H.startsWith("custom:")) {
    // ... custom model 检查 ...
    return {allowed: !0, isCustomModel: !0}  // custom → 允许
  }
  if (!kcH(H, $))
    return {allowed: !1, reason: bwH, isCustomModel: !1};  // 策略拒绝
...
**效果**:
- 所有非 `custom:` 前缀的模型（即 droid 官方内置模型）返回 `allowed: false`
- BYOK custom model 不受影响（走上面的 `custom:` 分支）
- `/model` 菜单中内置模型不可选
- `hasAnyAvailableModel` 仍返回 true（先查内置失败，再查 custom 成功）
- `getModel` fallback 逻辑：内置模型不可用 → 自动切到第一个可用 custom model

**字节**: 0 bytes (`!0` → `!1`，等长)

**稳定锚点**: `isCustomModel:!1};return{allowed:` + `isCustomModel:!1}}` — 字符串常量，不受混淆影响

### 修改 9: 禁用自动更新

**目标**: 阻止 droid 启动时自动检查和下载新版本。
...
**修改**: 在函数体开头插入 `return null;`，用注释填充到等长：
```javascript
async checkForUpdates(){return null;/*                   */if($.apiUrl)H=...}
```

**原理**: `checkForUpdates()` 返回 `null` 表示"无可用更新"，调用方 `$WD()` 收到 null 后
走 `if(!E)return "no-update"` 分支，跳过整个下载和安装流程。

**字节**: 0 bytes（`return null;/*..*/` 与原语句等长）

**稳定锚点**: `async checkForUpdates(){` — 方法名是语义化的，不受混淆影响。

**排查**:
```bash
# 检查方法是否存在
strings ~/.local/bin/droid | grep "async checkForUpdates"
# 检查是否已 patch
strings ~/.local/bin/droid | grep "checkForUpdates(){return null"
```

### 修改 10: Compaction 默认模型走 BYOK

**目标**: 避免上下文压缩走 Factory 后端，BYOK 用户压缩扣 Factory token 触发 402。

**位置**: `getCompactionModelMode()` fallback 默认值

**原始代码**:
```javascript
getCompactionModelMode(){return this.settings.general?.compactionModelMode??"factory-default"}
```

**修改**: `??"factory-default"` → `??"current-model"  ` (末尾补 2 空格保持 19 字节等长)

```javascript
getCompactionModelMode(){return this.settings.general?.compactionModelMode??"current-model"  }
```

**原理**: Summarizer 按 `getCompactionModelMode()` 决定压缩模型：
- `"factory-default"` → `eaH`（"claude-sonnet-4-5-20250929"，走 Factory 后端，扣 Factory token）
- `"current-model"` → 取当前对话模型（BYOK custom model，走用户自己的 key）

**字节**: 0 bytes（19 字节等长替换）

**稳定锚点**: `compactionModelMode??"factory-default"` — 字符串常量。

**排查**:
```bash
# 原版
strings ~/.local/bin/droid | grep 'compactionModelMode??"factory-default"'
# 已 patch
strings ~/.local/bin/droid | grep 'compactionModelMode??"current-model"'
```

**可选**：同步把 `~/.factory/settings.json` 的 `general.compactionModelMode` 显式设为 `"current-model"`，
双保险（即使二进制被官方覆盖升级也不会回退）。

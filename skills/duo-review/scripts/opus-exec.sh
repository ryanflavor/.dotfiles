#!/bin/bash
# 调用 Opus (Claude Opus 4.5) 执行 PR 审查
# 用法: opus-exec.sh PR_NUMBER=77 REPO=OrdoAI/ordo_ai OPUS_COMMENT_ID=xxx BASE_BRANCH=main
...
3. Post review: echo \"\$REVIEW_CONTENT\" | \$S/edit-comment.sh ${OPUS_COMMENT_ID}
...
\$S/duo-set.sh ${PR_NUMBER} s1:opus:conclusion <ok|p0|p1|p2|p3>"
